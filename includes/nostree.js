...
function NTreeFormat( fmt, tree )
{
	this.init = function( fmt, tree )
	{
		...
		this.pg = fmt[13][0];
		this.sp = fmt[13][1];
		this.SepColor = fmt[14];
		this.SepWidth = fmt[15][0];
		this.SepHeight = fmt[15][1];
		this.SepImage = fmt[16];
...
function NosTree( name, nodes, format )
{
	...
	this.expandNode = function( index )
	{
		...
		var Options = GetCookie(this.name);
		if (Options != null)
		{
			var pos = Options.indexOf(',' + index.toString() + ',');
			...
			if (node != null)
			{
				if (node.expanded)
				{
					if (pos != -1)
					{
						//lo saco
						//document.write(pos);
						Options = Options.substring(0, pos) + Options.substring(pos + 2 + index.toString().length, Option.length);
					}
				}
				else
				{
...
function NTreeNode( treeView, parentNode , text, url, target){
	...
	this.init = function(){
		var s = "";
		if (bw.ns4) {
			s = '<layer id="'+this.id()+'d" z-index="'+this.index+10+'" visibility="hidden">';

			if (this.parentNode != null && this.parentNode.index == this.index - 1)
				s += '<table width=\"' + this.treeView.fmt.SepWidth + '\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td height=\"'+ this.treeView.fmt.SepHeight +'\" bgcolor=\"'+ this.treeView.fmt.SepColor +'\"><img src=\"'+ this.treeView.fmt.SepImage + '\" width=\"1\" height=\"'+ this.treeView.fmt.SepHeight +'\"></td></tr></table>';

			s += this.getContent()+'</layer>';
		} else {
			s = '<div id="'+this.id()+'d" style="position:absolute;visibility:hidden;z-index:'+this.index+10+';">';
